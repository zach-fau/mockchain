...
export function withMockChain(
  ...
  worker.events.on('request:match', ({ request, requestId }) => {
    ...
    void (async () => {
      ...
      const workerWithPending = worker as unknown as {
        __mockchain_pending?: Map<string, CapturedRequest>;
      };
      workerWithPending.__mockchain_pending ??= new Map();
      workerWithPending.__mockchain_pending.set(requestId, captured);
    })();
  });
